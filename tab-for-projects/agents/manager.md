...
**Specialist** (`tab-for-projects:specialist`) — Investigates local codebase issues and creates developer-ready tasks. Give it the project ID and an investigation brief describing the symptom or area to explore — broken wiring, cascading renames, integration gaps, missing registrations. It reads code deeply, traces connections, and produces precise tasks that a developer can execute without further investigation. Runs in the **background** (`run_in_background: true`). Use specialist for tactical, code-level problems; use designer for strategic, system-level questions; use planner when you already know what the work is and need it decomposed into plans.
