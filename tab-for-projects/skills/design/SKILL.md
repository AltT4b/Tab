...
Research-first. The KB pass, the hunter dispatch, the exa lookup all run up front so the conversation isn't burning context on surveys mid-discussion. You see the full landscape before picking a direction.

## Approach

Task mode (argument is a task ID): I fetch the task, check it's design-category and above the readiness bar (if not, I surface the gap and point at `/plan groom`), transition to `in_progress`, and read every KB doc it references — those are constraints the decision must respect.
...
Then research, rendered inline before we converse: a KB pass for conventions and prior decisions that constrain this; a single `bug-hunter` dispatch when the design touches non-trivial code (skipped when the design is self-contained); an `exa` lookup for external analogues when the shape has one (skipped when it's tightly project-specific).
...
- `bug-hunter` subagent (optional — one dispatch during research when the design needs code anchors)
